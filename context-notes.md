...
## 2026-09-05 — Blender로 건물/연석 재제작 ("너무 로우폴리였어" 피드백)

### 사용자 요청
"블렌더로 도시 맵의 땅을 만들어주고 건물들도 다시 만들어줘 너무 로우폴리 였어" — 이전 체크포인트의 건물(박스+처마+문마커 3파츠짜리)이 지나치게 단순했다는 지적. 여전히 로우폴리/스타일라이즈드 미감은 유지하되, 실제 건축적 디테일(창문/문틀/기초/처마)이 있는 수준으로 올려달라는 요청으로 해석.

### 새 건물 설계: 개별 파츠 조립 방식 (bmesh 인셋 대신)
- bmesh로 벽면에 실제 창문 구멍을 뚫는(inset+extrude) 방식은 헤드리스 환경에서 결과를 미리보기 없이 한 번에 정확히 만들기 까다로워서(시각적 반복 없이 좌표를 다 손으로 계산해야 함) 리스크가 큼 — 대신 기존에 이미 성공적으로 썼던 "여러 개의 작은 프리미티브를 조합해 하나의 FBX로 묶어 내보내기" 패턴(Barn/WoodFence/MeleeWeapon과 동일)을 재사용.
- 창문 = 프레임(밝은 회색, 벽보다 약간 튀어나옴) + 유리(어두운 남색, 프레임보다 약간 안쪽/작게) 2파츠 조합 — 실제로 안으로 들어간 것처럼 보이진 않지만(진짜 inset이 아니라 겹쳐진 박스), 프레임의 그림자와 유리의 어두운 색 대비만으로도 조명 아래에서 충분히 "창문처럼" 읽힘. 문도 동일 원리(프레임+패널)+작은 현관 계단(Stoop) 추가.
- 지붕은 기존의 얇은 박스 대신 살짝 더 두껍고(0.08) 벽보다 넓은(1.08배) 파라펫 박스로 변경해 처마 실루엣을 더 뚜렷하게 함. 기초(Plinth)를 벽보다 넓고 얇게 바닥에 깔아 건물이 "땅에 박혀있는" 느낌을 줌.
- 전부 1x1x1 단위 규격(기존 `CityBuilding.fbx`와 동일)으로 만들어서, 기존 20개 건물 인스턴스의 위치/회전/스케일 값을 하나도 다시 계산할 필요 없이 그대로 재사용 — 모델만 교체하는 방식으로 작업량을 크게 줄임.

### [함정 재발] Blender→Unity 축 변환 부호를 감으로 고쳤다가 또 틀림
- 새 모델을 처음 만들 때 문/창문을 Blender +Y(양수)에 배치했는데, 임포트해보니 Unity Z가 음수로 나옴 — "반대인가보다" 하고 부호를 다시 뒤집었더니 이번엔 반대 방향으로 또 틀렸음. 결국 원래(Blender 음수 Y)가 맞았던 것으로 판명(Unity 좌표를 직접 찍어서 확인 후 원복).
- 이 프로젝트에서 이미 "Blender FBX axis_forward='-Z' 설정은 Blender -Y가 Unity +Z로 매핑된다(부호가 뒤집힌다)"는 사실을 지난번 헛간/사일로 제작 때 실측으로 확인해서 기록해뒀었는데, 이번에 그 기록을 다시 찾아보지 않고 "감"으로 판단했다가 또 틀림. **앞으로 Blender 좌표 부호가 헷갈릴 때는 반드시 (a) 이 노트에서 이전 실측 기록을 먼저 찾아보고, (b) 그래도 확신이 안 서면 추측하지 말고 임포트 직후 `transform.localPosition`을 코드로 직접 찍어서 확인할 것** — 이번에도 결국 그렇게 해서 잡았지만, 한 번 왕복(틀림→재수정→재확인)을 낭비함.

### 연석(Curb)
- L자 단면 2단 박스(낮은 립 + 높은 뒷단)로 기존의 밋밋한 사각 박스 연석을 교체. 메인 거리(기존에 있던 것 교체)뿐 아니라 교차로에도 처음으로 연석 42세그먼트(8유닛 단위 타일링) 추가(교차로는 지난 체크포인트에서 연석 없이 인도만 있었음).
- 세그먼트 단위로 타일링한 이유: 연석 모델이 "단위 길이 1"로 만들어져서 하나를 통째로 80~90유닛까지 늘리면 비례가 깨짐(너무 얇고 길게 늘어남) — 8유닛 정도 세그먼트로 나눠 반복 배치하는 편이 실제 연석처럼 자연스러움. 앞으로 타일형 Blender 에셋을 만들 때는 "단위 길이 1을 그대로 스케일만 키워서 쓸지" vs "적당한 세그먼트로 반복 배치할지"를 미리 정하고 진행할 것(이번엔 처음엔 안 정하고 시작했다가 뒤늦게 세그먼트 방식으로 전환).

### 보도블록(SidewalkPanel.fbx) — 만들었지만 아직 미적용
- 줄눈 홈이 있는 보도블록 모델도 Blender로 제작 완료했으나, 연석과 마찬가지로 세그먼트 타일링 배치 작업이 필요해서(현재 평평한 텍스처 인도가 이미 어느 정도 봐줄 만해서) 이번 체크포인트 범위에서는 적용을 미루고 다음으로 넘김. 에셋은 `Assets/Models/City/SidewalkPanel.fbx`에 이미 있으니 다음에 바로 배치 작업만 하면 됨.

### 검증
- 45도 스크린샷으로 건물 파사드에 실제 창틀+유리 격자가 뚜렷이 보임을 확인(이전 "박스+색깔 문양"과 확연히 다름). Play Mode 종합 회귀(건물 입구 왕복 전환/좀비 스폰/총 발사) 전부 정상, 콘솔 에러 0건.

## 2026-09-05 — 낮 밝기 개선 + 낮부터 시작 (지난 세션에 남겨둔 한계 해결)

### 사용자 요청
"낮에는 맵이 밝아지게 하자고 낮부터 시작되게 하자" — 지난 체크포인트에서 "절차적 스카이박스라 밤에 하늘까지 드라마틱하게 어두워지진 않는다"고 남겨뒀던 한계를, 이번엔 반대 방향(낮에 확실히 밝아지길 원함)에서 다시 지적받음.

### 해결: 스카이박스 `_Exposure`/`_SkyTint`/`_GroundColor`를 직접 낮/밤 보간
- `Skybox/Procedural` 셰이더의 `_Exposure`(전체 밝기 배율) 프로퍼티가 핵심 — 기본값 1.3에서 낮 1.6/밤 0.15로 `DayNightCycle.Update()`에서 매 프레임 보간. `_SkyTint`/`_GroundColor`도 같이 보간해 밤엔 남색 계열, 낮엔 밝은 하늘색 계열로.
- **[중요]** `RenderSettings.skybox`는 프로젝트의 공유 머티리얼 에셋(`Default-Skybox`)을 그대로 가리키고 있어서, 이 프로퍼티들을 직접 `SetFloat`/`SetColor`로 건드리면 **에셋 자체가 영구적으로 변경**됨(다른 곳에서도 이 스카이박스를 쓰면 같이 바뀌고, 에디터를 껐다 켜도 마지막 상태가 남음). `Start()`에서 `new Material(RenderSettings.skybox)`로 인스턴스를 복제한 뒤 `RenderSettings.skybox`에 재할당해서, 런타임에만 색이 바뀌고 원본 에셋 파일은 항상 원래 상태(중립 회색 기본값)로 유지되도록 함 — **셰어드 머티리얼을 런타임에 수정해야 할 때는 항상 이 "복제 후 교체" 패턴을 쓸 것**(이전에 가로등 전구 머티리얼에서도 `renderer.material`로 동일한 문제를 이미 한 번 피해간 적 있음 — 이번엔 `RenderSettings.skybox` 레벨에서 재발할 뻔한 걸 처음부터 인지하고 예방).
- 태양광 세기(1.1→1.5)/환경광(0.4→0.58)도 함께 상향해 스카이박스뿐 아니라 지면·건물 조명 자체도 뚜렷하게 밝아지도록 함.

### 시작 시점 변경
- `Start()`의 초기 `elapsed`를 `cycleDuration*0.15`(해질녘 근처)에서 `cycleDuration*0.25`(정오, `DayAmount=1`)로 변경 — 게임을 켜면 바로 가장 밝은 낮으로 시작.

### [함정] 스크립트의 필드 기본값을 바꿔도 씬에 이미 배치된 컴포넌트에는 반영 안 됨
- `DayNightCycle.cs`의 `public float dayLightIntensity = 1.5f` 처럼 C# 필드 기본값을 바꿔도, **씬에 이미 저장되어 있는 컴포넌트 인스턴스는 예전에 저장된 값(1.1 등)을 계속 들고 있음** — Unity는 컴포넌트가 처음 추가된 시점의 값을 직렬화해두고, 스크립트의 기본값 변경을 기존 인스턴스에 소급 적용하지 않음. Play Mode에서 확인했을 때 `DayAmount=1`인데도 `ambientLight`가 옛날 값(0.4,0.4,0.42)으로 나와서 발견함.
- 해결: `SerializedObject`로 씬에 있는 실제 컴포넌트의 필드 값을 직접 새 값으로 덮어써야 함(코드 기본값 변경과는 별개 작업). **앞으로 이미 배치된 컴포넌트의 public 필드 기본값을 스크립트에서 바꿀 때마다, 씬에 있는 기존 인스턴스도 `SerializedObject`로 따로 갱신해야 한다는 것을 기억할 것** — 이 프로젝트에서 스크립트 필드 관련 세 번째 유사 함정(첫 번째: NavMesh 에셋 재저장 필요, 두 번째: 씬 저장 타이밍, 세 번째: 이번 컴포넌트 필드 소급 미반영).

### 검증
- Play Mode에서 시작 직후 `DayAmount=1` 확인. 낮/밤 스크린샷 비교 — 낮은 하늘이 밝은 파스텔 그라데이션+건물 색이 선명, 밤은 하늘이 확실히 짙은 남색으로 가라앉고 가로등만 도드라짐. 콘솔 에러 0건.

## 2026-09-05 — 건물이 플레이어를 가리면 반투명 처리

### 사용자 요청
"건물에 플레이어가 가려지면 그건물을 반투명하게 해서 카메라를 가리지 않게 해줘" — 탑다운/아이소메트릭 카메라 특성상 건물이 플레이어와 카메라 사이에 낄 수 있는 문제.

### 설계
- 새 `Building` 레이어(14)를 만들어 건물 20개의 `Building Body` 콜라이더에만 적용 — 레이캐스트를 이 레이어로 한정해서 플레이어/좀비/아이템 등 다른 콜라이더와 절대 안 섞이게 함(레이어 없이 태그나 이름 비교로 필터링하면 매 프레임 문자열 비교가 필요해 비효율적 — CLAUDE.md의 "Update에서 태그 검색 금지" 원칙과도 부합).
- `PlayerOcclusionFader.cs`: 카메라→플레이어 레이(`RaycastNonAlloc`, GC 없음)로 매 프레임 검사. 맞은 콜라이더의 부모(=건물 루트, 콜라이더는 항상 건물 루트의 직계 자식인 `Building Body`에 있으므로 `transform.parent`만으로 충분 — 별도의 재귀 탐색 불필요)를 추적.
- **공유 머티리얼 보호**: 건물들이 색상 재질(Beige/Brick/Concrete 등)을 서로 공유하고 있어서, 가려질 때 그 재질을 직접 고치면 같은 색을 쓰는 **다른 모든 건물**까지 같이 투명해지는 버그가 날 뻔함 — `renderer.material`(공유 아님, 최초 접근 시 자동으로 인스턴스 복제)로 접근해서 그 건물의 그 렌더러만 독립적으로 바뀌도록 함(가로등 전구·스카이박스에서 이미 두 번 겪은 것과 동일한 패턴, 이번엔 처음부터 인지하고 예방).
- **URP Opaque→Transparent 런타임 전환**: `_Surface`(0=Opaque/1=Transparent), `_SrcBlend`/`_DstBlend`(Alpha 블렌딩), `_ZWrite`(0), `_ALPHABLEND_ON` 키워드, `renderQueue`를 코드로 직접 설정 — Simple Lit 셰이더에 이 프로퍼티들이 전부 존재함을 먼저 `ShaderUtil.GetPropertyName`으로 확인한 뒤 진행(추측하지 않고 실제 프로�퍼티 목록을 조회).
- 한 번 투명 모드로 전환한 머티리얼은 완전히 복귀(알파=1)해도 Opaque로 다시 안 돌려놓음(성능상 상태 재전환 반복을 피하기 위한 의도적 단순화) — 알파 1인 투명 머티리얼은 시각적으로 불투명과 거의 구분 안 되고, 페이드 대상이 되는 건물은 소수라 z-write 없음으로 인한 부작용은 무시할 만한 수준으로 판단.

### 검증 팁: 실제 게임 카메라(Cinemachine)로는 오클루전 테스트 프레이밍이 어려움
- Follow Cam(Cinemachine)의 오프셋이 플레이어 기준 (-8,16,-8)로 고정되어 있어서, 건물이 "카메라와 플레이어 사이"에 오려면 플레이어가 건물 바로 옆(2~3유닛 이내)에 있어야 함 — 이 경우 카메라 자체가 건물에 거의 붙어버려 스크린샷이 건물 벽 클로즈업처럼 나와 페이드 효과를 육안으로 확인하기 어려웠음.
- 검증을 위해 **Main Camera의 transform을 헤드리스 테스트 중에만 수동으로 원하는 각도에 배치**(`cam.transform.position=...; cam.transform.LookAt(...)`)하고, `PlayerOcclusionFader.Update()`를 리플렉션으로 직접 호출해 그 시점 카메라 위치 기준으로 오클루전을 재계산시킨 뒤 스크린샷 — Cinemachine이 다음 프레임에 다시 정상 오프셋으로 되돌려놓으므로 실제 게임 동작에는 전혀 영향 없이 검증만 가능. **카메라 오프셋이 좁아 자연스러운 오클루전 테스트 프레이밍이 안 나올 때는 이 패턴(카메라 수동 배치 + 해당 컴포넌트 Update 직접 호출 + 스크린샷)을 쓸 것.**

### 검증 결과
- 리플렉션으로 강제 오클루전 상황을 만들어 확인: 감지된 건물의 `_BaseColor.a`가 정확히 0.25로 내려감, 스크린샷으로 건물 벽 뒤 플레이어가 또렷이 보임(창문 등 디테일도 함께 반투명해짐, 전체 건물이 일괄 적용됨을 확인). 플레이어를 다시 멀리 이동시키면 알파가 1로 복귀하고 추적 목록에서 제거됨. 총 발사/좀비 스폰 회귀 없음, 콘솔 에러 0건.

## 2026-09-05 — 가로등 재제작 + 나무/산책로 빈공간 배치

### 사용자 요청
"블렌더로 만든 가로등을 충분히 설치하고 나무도 만들어 빈공간에 넉넉히 설치하고 산책로도 만들어 빈공간에 배치해줘".

### 가로등 재제작
- 기존 primitve 조립형(원기둥+박스 몇 개)을 Blender 제작 모델로 교체: 테이퍼진 원뿔형 기둥, 2단 박스로 곡선을 흉내낸 암, 원기둥+원뿔 캡으로 된 램프 하우징. 기존과 동일하게 `StreetLight.cs`(밤에만 점등, `renderer.material` 인스턴스로 다른 가로등과 독립적으로 색 전환)를 그대로 재사용.
- 메인 거리뿐 아니라 교차로에도 처음으로 가로등 배치(총 18개, 기존 11개에서 확대) — 교차로가 지금까지 밤에 완전히 캄캄했던 것도 같이 해결됨.

### 나무: 새로 안 만들고 기존 시골 나무 재활용 + 격자 산포 알고리즘
- "나무도 만들어"라고 했지만 이미 `Assets/Models/Rural/Tree.fbx`(지난 세션에 Blender로 만든 불규칙 수관 나무)가 있어서 재사용 — 효율적이고 이미 게임의 로우폴리 스타일에 맞음.
- 배치 알고리즘: 지형 전체를 8유닛 격자로 훑으면서 각 셀마다 ±2.5 지터를 준 후보점 생성 → 도로 코리도(메인 거리 Z -7~11, 교차로 X 51~69)와 건물 반경(6.5유닛) 안에 있으면 스킵 → 남은 후보의 25%를 추가로 무작위 스킵(완전 격자형으로 안 보이게) → 나머지에 나무 배치. 총 65그루, 도시 전역 빈 공간에 자연스럽게 분포.
- **[함정]** `GameObject.Instantiate(fbxAsset, parent)`로 배치했더니 재질이 기본 `Lit`(흰색)로 나옴 — `PrefabUtility.InstantiatePrefab`가 아니라 순수 `GameObject.Instantiate`를 썼기 때문이 아니라(둘 다 정상적으로 프리팹 계층을 복제함), **원본 FBX 에셋 자체에는 애초에 올바른 재질이 저장되어 있지 않다**는 게 근본 원인 — 이 프로젝트의 나무/농장 소품 워크플로우는 항상 "배치 후 매 인스턴스마다 재질을 코드로 지정"하는 방식이었지 에셋 자체에 재질을 굽어두는 방식이 아니었음(지난 시골 배치 때도 매번 재질을 지정했었는데, 이번엔 그 사실을 깜빡하고 "이미 재질이 있겠지"라고 가정했다가 65그루가 전부 흰색으로 나온 뒤에 발견). **Blender에서 가져온 FBX를 배치할 때는 항상 그 직후에 재질 지정 스텝이 필요하다는 것을 잊지 말 것** — 지금까지 매번 그렇게 해왔으면서도 이번에 한 번 빠뜨림.

### 산책로 (신규)
- `WalkingPath.fbx`(포장 슬래브+양쪽 가장자리 턱+중앙 이음선, 사이드워크 패널과 동일한 "단위 길이 1, 5유닛 세그먼트 타일링" 패턴) 제작.
- 북쪽 개활지에 동서로 뻗는 산책로, 남쪽 개활지에도 동일하게, 그리고 북쪽 산책로에서 메인 거리 인도까지 이어지는 남북 연결로 하나 — 총 22세그먼트. 완전히 직선(대각선/곡선 없이 축 정렬)으로만 구성해 회전 각도 계산 리스크를 피함(곡선/대각선 산책로는 다음에 시간 여유 있을 때 시도 가능 — 세그먼트 방향을 임의 각도로 회전시키려면 `Quaternion.LookRotation` 후 로컬 X축 정렬을 위해 추가로 ±90도 보정하는 계산이 필요, 이번엔 범위에서 제외).

### 검증
- NavMesh 재굽기+에셋 재저장(이제 이 패턴은 거의 자동으로 몸에 뱀 — 매번 새 오브젝트를 추가할 때마다 잊지 않고 수행 중). Play Mode 회귀(총 발사/좀비 스폰) 확인, 콘솔 에러 0건. 탑뷰 스크린샷으로 도시 전역에 나무가 고르게 퍼져있고 산책로가 지나가는 것을 확인.

## 2026-09-05 — 낮/밤 좀비 스폰 + 총소리 노이즈 시스템

### 사용자 요청
"아침에는 좀비가 없고 밤에는 좀비가 나오는데 총소리 처럼 큰소리가 있으면 좀비 떼가 몰려오게".

### 낮/밤 좀비 게이팅
- `ZombieSpawner.Update()`에서 `DayNightCycle.IsNight`를 확인해 밤에만 `SpawnWave()` 호출. 낮으로 전환되는 그 순간(직전 프레임 밤 → 이번 프레임 낮)에만 1회 `ClearAllZombies()` 실행 — 매 프레임 검사하면 낮 내내 계속 청소를 시도하게 되므로 반드시 "전환 감지"(`wasNight` 캐시) 방식으로 처리.
- `ClearAllZombies()`는 `zombie.Die()`를 거치지 않고 `Destroy(zombie.gameObject)`를 직접 호출 — `Die()`를 거치면 `onDeath` 이벤트 체인(`GameManager.AddScore(100)` 포함)이 발동해 아침이 될 때마다 좀비 마리수만큼 점수가 올라가는 의도치 않은 버그가 생길 뻔했음. 점수 없이 조용히 사라지는 것이 "아침엔 원래 없었다"는 컨셉에 맞음.

### 총소리 노이즈 시스템
- `NoiseManager`(정적 이벤트 허브) → `Gun.Shot()`에서 발사 시점마다 `EmitNoise(발사위치, 50유닛)` → `Zombie`가 구독해서 반경 안이면 `noiseTarget`/`noiseExpireTime`(12초) 저장.
- `Zombie.UpdatePath()` 코루틴 재구성: 기존엔 "타겟 없으면 제자리 정지 + 20유닛 반경 탐색"이었는데, 이제 "타겟 없으면 20유닛 탐색 먼저 시도 → 그래도 없으면 유효한 소리가 있는 동안 그 위치로 이동 → 둘 다 없으면 정지"로 확장. **소리를 쫓아가는 중에도 매 틱 정상 감지를 계속 시도**하게 만든 게 핵심 — 그래야 소리 위치로 걸어가다가 실제 플레이어를 마주치면 자연스럽게 진짜 추적으로 전환됨(안 그러면 소리 위치까지 걸어가서 딱 멈춰버리고 바로 옆의 플레이어를 못 알아채는 어색한 상황이 생김).

### [함정] NavMeshAgent가 붙은 오브젝트는 `transform.position` 직접 대입이 안 먹힘
- 좀비를 헤드리스 테스트로 "멀리 떨어뜨린 뒤 소리로 끌려오는지" 검증하려고 `zombie.transform.position = far`로 옮겼는데, 다음 프레임에 확인해보니 위치가 원래 근처로 돌아와 있었음(NavMeshAgent가 매 프레임 자신의 내부 시뮬레이션 위치로 transform을 덮어씀). 이 상태로는 좀비가 애초에 플레이어 근처에 있었으니 정상 감지로 잡힌 것뿐인데 "소리로 끌려왔다"고 착각할 뻔함.
- 해결: `NavMeshAgent.Warp(position)`을 사용해야 에이전트의 내부 상태까지 같이 갱신되어 실제로 이동함. **NavMeshAgent가 달린 오브젝트를 스크립트/테스트 코드로 순간이동시킬 때는 항상 `transform.position` 말고 `agent.Warp()`를 쓸 것** — 이 프로젝트에서 좀비 관련 헤드리스 테스트를 할 때마다 재발할 수 있는 함정이니 다음에도 기억할 것.
- 이 방식으로 재검증: 플레이어 반경 20(정상 감지 범위) 밖인 42유닛 지점에 `Warp`로 좀비를 배치 → 총 발사 → 그 좀비의 `noiseTarget`/`noiseExpireTime`이 정확히 설정되고 실제로 `NavMeshAgent.destination`이 소리 방향으로 갱신되며 이동을 시작함을 확인(정상 감지로는 절대 안 잡혔을 거리였다는 게 확실함).

### 검증
- 낮 시작(좀비 0) → 밤 전환(웨이브 1, 2마리 스폰) → 다시 낮 전환(즉시 전부 제거) 왕복 확인. 노이즈로 먼 좀비가 끌려오는 것 확인. 총 발사/플레이어 체력 정상, 콘솔 에러 0건.

### 다음 세션 참고
- 노이즈 반경(50)과 조사 유지 시간(12초)은 감으로 정한 초기값 — 실제 플레이해보고 "너무 멀리서도 몰려온다"/"금방 포기한다" 싶으면 `Gun.noiseRadius`, `Zombie.noiseInvestigateDuration` 인스펙터 값만 조정하면 됨(코드 수정 불필요).
- 근접무기(삽) 공격은 현재 노이즈를 발생시키지 않음(총소리만) — 사용자가 원하면 `MeleeWeapon.DetectHit()`에도 `NoiseManager.EmitNoise` 호출을 추가하면 됨(반경은 총보다 작게, 예: 15유닛 정도가 자연스러울 것).

## 2026-09-05 — 총소리 시 화면 밖 좀비 떼 스폰 (사용자 설명 보완)

### 사용자 재설명
직전 체크포인트(CP26, 기존 좀비가 소리 쪽으로 끌려오기만 함)에 대해 "설명이 부족했다"며 명확히 함 — 원했던 건 총소리가 나면 **새 좀비 무리(최소 50마리)가 화면 밖 근처에서 스폰되어** 플레이어에게 몰려오는 것, 소리가 계속되면(연속 사격) 계속 스폰되어 **최대 200마리**까지 늘어나는 것.

### 설계: 기존 메커니즘과 상호 보완
- CP26의 "기존 좀비가 소리 쪽으로 끌려옴" 로직은 그대로 유지(삭제 안 함) — 이번 신규 기능(새 좀비를 화면 밖에서 스폰)과 상충하지 않고 서로 보완됨(이미 필드에 있던 좀비도 반응하고, 추가로 새 무리도 몰려옴).
- `ZombieSpawner`가 `NoiseManager.OnNoiseEmitted`를 직접 구독 — 소리가 나면(밤+게임오버 아님 조건일 때) `hordeBurstSize`(50)와 `maxZombiesAlive`(200) 잔여분 중 작은 값만큼 코루틴으로 나눠서 스폰.

### 화면 밖 위치 계산
- 플레이어를 중심으로 각도를 돌려가며, 거리를 12유닛부터 3유닛씩 늘려가며 `Camera.main.WorldToViewportPoint`로 뷰포트 좌표가 [0,1] 범위(약간의 여유 margin 포함) 밖으로 나가는 첫 지점을 찾음 — 순수 반지름 고정이 아니라 **실제 카메라 프러스텀 기준으로 화면 밖인지 판정**하기 때문에, 카메라 각도/줌이 바뀌어도 항상 정확히 "화면에 안 보이는 가장 가까운 지점"에서 스폰됨.
- 찾은 지점을 그대로 쓰지 않고 `NavMesh.SamplePosition`으로 보정 — 화면 밖 지점이 건물 안이나 지형 밖에 걸릴 수 있어서, 실제로 걸을 수 있는 가장 가까운 NavMesh 위 지점으로 스냅.
- 무리 전체는 플레이어 주변 360도에 균등 각도+지터로 분산 배치 — 한쪽에서만 오지 않고 사방에서 포위하듯 몰려오는 효과.

### 대량 생성 성능 고려
- 50~200마리를 한 프레임에 다 만들면 순간 프레임 드랍이 생길 수 있어서, 코루틴으로 프레임당 12마리씩만 생성하고 나머지는 다음 프레임으로 넘김(`SpawnHordeRoutine`). 아직 실측 프로파일링은 안 했음 — CLAUDE.md 원칙대로 "일단 단순하게 구현, 문제 생기면 그때 측정 후 최적화"로 처리. 다음에 실제로 200마리가 동시에 존재할 때 프레임 저하가 체감되면, 오브젝트 풀링이나 프레임당 생성 수 조정(`hordeSpawnPerFrame` 인스펙터 값)부터 시도할 것.

### 즉시 추적을 위한 `Zombie.ForceTarget()`
- 화면 밖에서 스폰된 좀비가 기존 방식(0.25초마다 반경 20 안에서 대상 탐색)을 기다리면, 플레이어와 멀리 떨어진 스폰 지점 특성상 정상 탐지로는 절대 못 찾음 — `ForceTarget(player)`으로 스폰 즉시 강제로 추적 대상을 지정해서 바로 달려오게 함.

### 검증 (및 사소한 테스트 아티팩트)
- 밤 강제 전환 → 총 발사 → 정확히 50마리가 전부 화면 밖(뷰포트 범위 밖)에 스폰되고 전부 타겟이 플레이어로 강제 지정됨을 확인. 연속 발사로 스포너 내부 리스트(`zombies` 필드, 직접 리플렉션 조회)가 정확히 200에서 멈춤을 확인(더 쏴도 안 늘어남 — 상한 로직 정상).
- **[사소한 테스트 아티팩트, 실제 버그 아님]** `GameObject.FindObjectsByType<Zombie>()`로 센 값은 203으로, 스포너 내부 리스트(200)보다 3 많게 나옴 — 이번 세션에서 리플렉션으로 `Update()`를 수동 호출하는 동시에 `EditorApplication.Step()`으로도 자연스러운 `Update()`가 같이 도는 등 헤드리스 테스트 특유의 중복 호출 때문으로 추정(스포너 자체의 상한 로직은 내부 리스트 기준으로 정확히 동작함을 확인했으므로 실제 게임플레이에는 영향 없음). 다음에 유사한 대량 스폰 테스트를 할 때는 리플렉션 수동 호출과 `Step()`을 섞어쓰지 말고 한쪽만 사용할 것.
- 콘솔 에러 0건, 200마리 이상 존재하는 상태에서도 플레이어 체력 정상(hp=100 유지, 아직 좀비들이 도달 전이라 당연함).

## 2026-09-05 — 세션 종료 (하루 전체 요약, 다른 컴퓨터에서 이어서 작업 예정)

오늘 하루 동안 맵 방향이 "묘지+농장"에서 "도시→광산→방공호→시골" 대형 아포칼립스 맵으로 완전히 전환되었고, 도시 구역만으로도 CP13~CP27까지 총 15개 체크포인트를 진행함(전부 커밋·검증·푸시 완료). 사용자가 피곤해서 오늘은 여기까지 하기로 함.

### 오늘 완료된 것 (커밋 순서대로, 전부 `origin/main`에 push 완료)
1. 씬 전환 시스템(`SceneTransitionManager`/`BuildingEntrance`/`InteriorExit`, Additive 로드) + 도시 1차 슬라이스(건물 10동).
2. 도시 확장(건물 14동) + Store Interior 추가.
3. 바닥/도로/건물 랜드마크(경찰서/소방서/병원/약국/아파트) + House Interior를 거실+부엌+화장실+침실2로 재설계.
4. 낮/밤 순환 + 가로등 + 실제 "+"자 교차로(건물 20동으로 확장).
5. Blender로 건물/연석을 상세 모델(18~20파츠)로 재제작 — "너무 로우폴리" 피드백 반영.
6. 보도블록(줄눈) 타일링 배치.
7. 낮 밝기 개선(스카이박스 exposure 보간) + 정오부터 시작.
8. 건물이 플레이어를 가리면 반투명 처리(`PlayerOcclusionFader`).
9. 가로등 재제작(Blender) + 나무 65그루 + 산책로 빈공간 배치.
10. 부서진/고장난 차량 3종(Blender) 8대 도로 배치.
11. 낮/밤 좀비 스폰 게이팅(아침엔 없음, 밤에만 스폰) + 총소리 노이즈로 기존 좀비 유인.
12. 총소리 시 화면 밖에서 새 좀비 무리 스폰(최소 50~최대 200마리) — 사용자가 실제 플레이로 확인, 버벅임 없음.

### 최신 상태
- 최신 커밋: `ae6bec1`, 브랜치 `main`, `origin/main`과 완전히 동기화됨(`git status --short` 깨끗함, Packages/ProjectSettings의 사소한 로컬 환경 차이만 남아있고 이는 매 세션 공통으로 커밋 대상 아님).
- 콘솔 에러 0건 상태 유지. Play Mode에서 기존 기능(총/근접무기/인벤토리/미니맵/상태창/건물 입구 전환) + 오늘 추가된 전부(낮밤전환/가로등/오클루전 페이드/좀비 게이팅/노이즈 호드) 회귀 검증 완료.

### 다음 세션 시작 시 순서
1. `plan.md` 최상단 "현재 상태 요약" 섹션 먼저 읽기.
...
## 2026-09-06 — 도로/바닥이 완전히 검게 렌더링되는 버그 진단/수정 (매우 긴 디버깅)

### 증상
사용자 리포트: "도시 영역 바닥이 검정인데 너무 검정이야 실제 도시 바닥은 검정이 아니잖아". Play Mode 스크린샷으로 확인해보니 `Main Road`/`Cross Road`/`Ground` 표면이 차선(Lane Dash)이나 자갈(Rubble) 등 위에 놓인 소품을 빼면 완전히 순수 검정(#000000에 가까움)으로 렌더링되고 있었음.

### 기각된 가설들 (전부 실측으로 확인 후 기각)
1. **DayNightCycle 시간 문제** — Play Mode 중 실제 시간이 계속 흘러 밤에 가까워진 것뿐이라 추측했으나, 리플렉션으로 정오(`DayAmount=1`)로 강제 고정해도 동일하게 검게 나옴.
2. **포스트프로세싱(ColorAdjustments contrast=6, ACES 톤매핑)** — 포스트프로세싱을 완전히 꺼도, contrast를 0으로 낮춰도, 톤매핑 모드를 None/Neutral로 바꿔도 변화 없음(단, 이 과정에서 카메라 위치를 여러 tool 호출에 걸쳐 두면 Cinemachine이 실시간으로 플레이어를 따라가며 카메라를 되돌려놔서 서로 다른 위치를 비교하는 실수를 함 — **여러 조건을 비교할 땐 반드시 하나의 `execute_code` 호출 안에서 카메라를 한 번만 배치하고 연속으로 렌더링할 것**, 여러 호출에 걸치면 실시간 게임 시스템이 그 사이에 상태를 되돌려놓을 수 있음).
3. **Ground/Tiles 오브젝트가 Static+Contribute GI(라이트맵) 상태였던 것** — 확인해보니 `Ground`/`Tiles`가 실제로 `staticFlags=2147483647`(전부 체크), `lightmapIndex` 할당됨, `receiveGI=Lightmaps`였음. 실시간 낮/밤 시스템과 안 맞는 설정이라 고쳤지만(`receiveGI=LightProbes`, `lightmapIndex=-1`), **막상 고쳐도 시각적으로 전혀 달라지지 않음** — 나중에 알고보니 그 위에 `Main Road`가 겹쳐 렌더링되고 있어서 Ground 자체를 눈으로 확인할 수 없었던 것이었고(테스트 방법론 실수), 최종적으로는 이 static/라이트맵 설정이 실제 원인이 아니었음이 밝혀짐(진짜 원인 수정 후에는 이 설정을 원래대로 되돌려도 정상적으로 밝게 나옴 — **그대로 두었음, 불필요한 변경 안 함**).
4. **머티리얼 `_BaseColor` 밝기 문제** — `_BaseColor`를 0.9(거의 흰색)까지 올려도 전혀 안 밝아짐. 심지어 태양 강도를 20, 환경광을 (3,3,3)까지 올리는 "핵폭탄급" 테스트를 해도 완전히 무반응(다른 오브젝트인 자갈/차선/새로 만든 테스트 큐브는 전부 하얗게 날아갈 정도로 밝아졌는데 도로만 그대로 순수 검정) — **이 시점에서 "재질이 어떤 값을 곱하든 0이 되는 무언가가 있다"는 확신을 가짐**(0 × 아무리 큰 수 = 0이라는 곱셈 특성과 정확히 일치).
5. **Shadow(그림자)** — `sun.shadows = None`으로 그림자를 완전히 꺼도 동일하게 검음. 그림자 문제 아님.
6. **렌더 파이프라인/카메라 스택/렌더러 피처** — Main Camera는 Base 타입, 스택 없음. URP Renderer Data에 등록된 Renderer Feature 0개. Terrain 컴포넌트도 씬에 없음. 전부 정상.

### 진짜 원인을 찾은 결정적 테스트
같은 위치에 **완전히 새로운 프리미티브 큐브**를 만들어 기존에 잘 작동하는 재질(`Building Beige`, 텍스처 없음)을 입혔더니 **완벽하게 정상적으로 밝게 렌더링됨** — 이걸로 "위치/전역 조명" 문제가 아니라 "그 오브젝트/재질 자체" 문제임을 확정. 이어서 같은 새 큐브에 `Road Asphalt` 재질을 입혔더니 **똑같이 순수 검정으로 재현됨** — 재질이 원인임을 완전히 확정.
`Road Asphalt`/`City Ground` 재질과 정상 작동하는 `Building Beige` 재질의 모든 셰이더 프로퍼티(Surface/ZWrite/Cull/Smoothness/SpecColor/GI 플래그 등)를 전수 비교했으나 전부 동일 — 유일한 차이는 `Road Asphalt`/`City Ground`에는 `_BaseMap`/`_MainTex`에 각각 `Asphalt.png`(40x5 타일링)/`CityGround.png`(45x45 타일링)라는 64x64 노이즈 텍스처가 물려있고, `Building Beige`는 텍스처가 아예 없다(플랫 컬러만 사용)는 것.
`_BaseMap`을 `Texture2D.whiteTexture`로 강제 교체했더니 **즉시 정상적으로 밝게 렌더링됨** — 텍스처가 실제 원인임을 확정. 텍스처 파일(`Asphalt.png`/`CityGround.png`)을 직접 열어보면 순수 검정이 아니라 어두운 회색 노이즈처럼 보이는데, 어째서 엔진 내에서 완전한 (0,0,0)으로 샘플링되는지(sRGB 설정/압축 포맷/저해상도+고배율 타일링 조합 등 여러 가능성 추정만 하고) 100% 확실한 근본 원인까지는 밝히지 못함 — 다음에 시간 여유가 있으면 텍스처 임포트 설정(`textureCompression`/`compressionQuality`)을 바꿔가며 재현 실험을 해볼 수 있음.

### 수정
이 프로젝트의 다른 모든 재질(Trim Gray/Door Dark/Building Beige/Roof Dark 등)이 전부 텍스처 없이 플랫 컬러만 쓰는 스타일이므로, 문제의 텍스처를 완전히 제거하고 동일한 방식으로 통일 — `Road Asphalt`: `_BaseColor` 0.12→0.20(어두운 아스팔트 그레이), `City Ground`: 0.24→0.42(밝은 포장 그레이). ACES 톤매핑이 어두운 값을 뭉갠다는 걸 감안해 실측 스크린샷으로 눈에 자연스러운 밝기까지 반복 조정함. 텍스처 파일(`Asphalt.png`/`CityGround.png`) 자체는 다른 곳에서 참조하는지 확인하지 않았으므로 삭제하지 않고 남겨둠.

### 디버깅 함정 정리 (다음에 유사 버그 만나면 참고)
- **여러 렌더링 조건을 비교할 때는 반드시 하나의 스크립트 실행 안에서 카메라를 한 번만 배치하고 연속으로 캡처할 것.** 서로 다른 `execute_code` 호출 사이에는 실제 게임 시간이 흐르고 Cinemachine 같은 실시간 시스템이 카메라/오브젝트 상태를 되돌려놓을 수 있어, 다른 시점의 장면을 비교하는 줄도 모르고 비교하는 실수가 발생하기 쉬움.
- **재질 색상을 바꿔도 화면이 안 바뀌면, 그 재질이 진짜 그 화면 위치에 보이는 게 맞는지부터 의심할 것.** `Physics.Raycast`는 콜라이더가 없는 오브젝트(이번의 `Main Road`처럼)를 그냥 통과해버려서 실제로는 다른(더 안쪽/아래) 오브젝트를 가리키고 있었음 — 화면 좌표 기반 확인(`ScreenPointToRay`)도 커스텀 `RenderTexture` 해상도와 카메라의 실제 `pixelWidth/Height`가 다르면 좌표가 어긋나므로, `ViewportPointToRay`(0~1 정규화 좌표)를 쓰거나 아예 오브젝트의 `Renderer.bounds`로 직접 겹치는지 확인하는 편이 안전함.
- **"조명 값을 아무리 올려도 안 밝아진다"는 극단적 테스트(태양 강도 20, 환경광 3 등)는 "그 표면이 정말 값을 전혀 반영 못 하고 있다"를 확정하는 데 매우 효과적**이었음 — 어중간한 값으로는 "약간 어둡네" 정도로만 보이고 확신을 못 가졌을 텐데, 극단값에서도 완전히 무반응인 걸 보고 나서야 "무언가에 0이 곱해지고 있다"는 결정적 단서를 얻음. 애매한 렌더링 버그를 조사할 때는 파라미터를 정상 범위를 훨씬 넘는 극단값으로 밀어붙여서 반응 여부만 이분법적으로 확인하는 방법이 유용함.
- **같은 위치에 새 오브젝트/다른 재질을 즉석에서 만들어 대조군으로 삼는 것**이 위치 문제 vs 오브젝트 문제 vs 재질 문제를 구분하는 가장 빠른 방법이었음 — 헤드리스 환경에서 프리미티브(`GameObject.CreatePrimitive`)를 즉석에서 만들고 지우는 건 비용이 거의 없으므로 의심되는 변수 하나만 남기고 나머지를 전부 "정상으로 알려진" 것으로 치환해보는 대조 실험을 적극적으로 쓸 것.

### 검증
Play Mode에서 정오 조명 강제 후 스크린샷 비교 — 수정 전(도로 전체가 순수 검정, 차선/자갈만 보임) vs 수정 후(명확히 구분되는 회색 아스팔트/포장 바닥) 확인. GI/라이트맵 설정을 원래 상태(static+lightmap 그대로)로 되돌려도 정상적으로 밝게 렌더링되는 것까지 확인해, 그 설정이 실제 원인이 아니었음을 최종 확인(불필요한 변경 하지 않음). 컴파일 에러 없음(재질 에셋만 수정), 콘솔 에러 0건.

## 2026-09-06 — 아파트 건물 전용 모델 제작 (CP29 모델이 고층 비율에서 깨짐)

### 문제 진단
사용자 리포트: "apartments 건물이 재대로 안되". Play Mode 스크린샷으로 실측한 결과, `Apartments Building South 3`/`North 4`의 루트 `localScale.y`가 각각 15(다른 저층 건물은 Y=4~7 정도)임을 확인 — CP29에서 만든 `BuildingDetailed.fbx`는 창문이 2행(1층+2층)만 있는데, 이 모델을 Y=15로 늘리면 창문 2개가 세로로 거대하게 늘어난 슬릿처럼 보이고 층 구분(벨트코스도 1개뿐)이 전혀 없어 "건물처럼 안 보이는" 상태였음. 스크린샷으로 실제로 확인해보니 정말 창문 2개가 위아래로 쭉 늘어난 흉한 모습이었음(모서리 필라스터/지붕/파라펫 등 스케일 무관한 파츠는 문제없었음 — 순전히 "행 개수가 고정된 채 세로로만 늘어나는" 창문/벨트코스가 문제).

### 해결: 아파트 전용 5층 모델
공유 FBX 하나로 모든 건물(짧은 것/키 큰 것)을 다 커버하려는 대신, 아파트 2개 전용으로 `BuildingDetailedTall.fbx`를 새로 제작 — 몸체/창틀/유리를 5행(1층 문 자리 포함, 나머지 4개 층)으로 재구성하고, 벨트코스도 층 경계마다(4개) 배치. 문/캐노피/모서리 필라스터/지붕/파라펫/환기구/기초/계단은 CP29 모델의 기존 오브젝트를 그대로 재사용(이 파츠들은 "1층/전체 높이" 개념이라 스케일이 세로로 늘어나도 원래부터 문제없었음 — 오직 반복되는 창문 행 개수만 하드코딩돼 있던 게 문제였음).

### [함정] Boolean 컷터를 한 번에 60개 합쳐서 처리하면 결과가 완전히 깨짐
CP29에서는 24~25개 컷터를 하나로 합친 뒤 Boolean Difference 한 번으로 처리해 문제없었는데, 이번에 5행(60개 컷터)으로 늘리자 같은 방식으로 처리한 결과 **몸체 메시가 정점 0개로 완전히 사라짐**(Boolean 솔버가 복잡한 다중 컷터 조합에서 실패한 것으로 추정). **면(앞/뒤/좌/우) 4개로 나눠 컷터를 그룹화하고 Boolean을 4번 순차적으로 적용**하니 정상 작동(최종 496 정점). **앞으로 Boolean으로 뚫어야 할 구멍(컷터) 개수가 많아질 때는(대략 30개 이상?) 한 번에 다 합쳐서 처리하지 말고, 논리적 그룹(면별/층별 등)으로 나눠 여러 번의 작은 Boolean으로 순차 적용할 것** — 실패해도 정점 개수가 0인지 확인하는 습관을 들이면 이런 실패를 바로 감지할 수 있음(이번엔 다음 스텝에서 재질을 입히려다 `body.data.vertices`가 비어있는 걸 보고 알아챔).

### 마이그레이션: 이번엔 콜라이더 함정 재발 안 함
CP29에서 겪었던 "새 FBX 파츠의 localScale=100 때문에 콜라이더가 100배 커지는" 함정을 이번엔 처음부터 `MeshFilter.sharedMesh.bounds`로 콜라이더 크기를 설정해 재발하지 않음(교훈이 실제로 다음 작업에 적용된 사례).

### 검증
아파트 2개만 새 모델로 교체(나머지 18개 저층 건물은 CP29 모델 그대로 유지 — 굳이 다 바꿀 필요 없음). 재질 슬롯 null 0건, 파츠 개수 정확히 13개, 콜라이더 월드 크기가 교체 전과 동일(5.5x5.5x13.5, 7x6x13.5 — 기존 NavMesh 그대로 재사용 가능, 재굽기 불필요). Play Mode 스크린샷으로 5층 창문+층별 벨트코스가 비율에 맞게 자연스럽게 보이는 것 확인. 콘솔 에러 0건.

## 2026-09-06 — 건물 오클루전 페이드 알파값 조정

### 사용자 요청
"플레이어가 건물 뒤로 가면 건물이 투명도가 많이 반투명해져서 플레이어가 안가려지도록 하는걸 고치자" — 기존 `PlayerOcclusionFader.fadedAlpha=0.25`로는 특히 채도 높은 벽 색상(Police Blue 등)에서 여전히 플레이어가 잘 안 보인다는 지적.

### 수정
`fadedAlpha` 0.25 → 0.15로 낮춤. 씬의 컴포넌트 인스턴스 값을 `SerializedObject`로 직접 수정(스크립트 필드 기본값이 아니라 이미 씬에 저장된 값을 갱신해야 함 — "낮 밝기 개선" 체크포인트 때 같은 함정을 이미 겪었으므로 이번엔 처음부터 `SerializedObject` 사용).

### 검증 관련 시행착오
플레이어 캐릭터를 강제로 건물 뒤에 이동시켜 스크린샷으로 시각 확인하려 했으나, `Rigidbody`(물리 충돌로 위치가 밀려남)와 `PlayerMovement`(잔여 입력/애니메이터 상태)가 계속 개입해 의도한 위치에 얌전히 있지 않았고, 카메라 `LookAt` 대상 좌표 계산에서도 반복적으로 실수해(건물 로컬 오프셋과 월드 좌표를 헷갈림) 결국 깔끔한 비교 스크린샷은 확보하지 못함. 대신 재질의 `_BaseColor.a` 값이 정확히 0.25→0.15로 수렴하는 것을 반복적으로 직접 확인해 메커니즘 자체는 확실히 검증함(알파 블렌딩 수식은 값이 낮을수록 무조건 더 투명해지는 게 자명하므로, 스크린샷 없이도 수치 검증만으로 충분하다고 판단).
**다음에 플레이어를 강제 이동시켜 검증할 때는**: `PlayerMovement.enabled=false` + `Rigidbody.isKinematic=true`로 완전히 멈춘 뒤 위치를 설정할 것(이번에 이렇게 했음에도 카메라 프레이밍 실수는 별개로 발생했음 — LookAt 타겟은 항상 "월드 좌표" 기준으로 명확히 계산하고, 건물 중심 기준 로컬 오프셋을 쓸 때는 어느 축이 카메라 쪽/반대쪽인지 먼저 확인할 것).

## 2026-09-06 — 건물 오클루전 페이드 추가 하향 (0.15→0.06)

### 사용자 피드백
"반투명해지기는 한데 건물이 반투명해져도 잘보여" — CP31에서 0.25→0.15로 낮췄지만 실제 플레이해보니 여전히 건물이 뚜렷하게 보임.

### 시도했으나 신뢰하기 어려웠던 검증 방법
헤드리스 스크린샷으로 알파 0.15 상태를 진단하려고 카메라를 건물 근처 눈높이(y≈4)에 배치해봤는데, 이 각도에서는 건물 실루엣이 화면의 절반 이상을 차지해버려서(실제 게임은 Cinemachine이 훨씬 가파른 탑다운 각도로 따라감 — 기존 오프셋 (-8,16,-8) 참고) "반투명해도 건물 형태가 뚜렷이 남는다"는 사용자의 체감과 비슷한 그림이 나오긴 했지만, 이 프레이밍 자체가 실제 게임 카메라와 달라서 알파값을 정량적으로 비교하는 용도로는 신뢰하기 어려웠음. **탑다운 게임에서 오클루전 페이드처럼 카메라 앵글에 크게 좌우되는 시각 효과를 헤드리스로 검증할 때는, 실제 게임의 Cinemachine 오프셋과 최대한 비슷한 각도로 카메라를 배치해야 유의미한 비교가 됨** — 이번엔 시간 관계상 정확히 재현하지 못하고 수치 검증(재질 알파가 목표값에 정확히 도달하는지)만으로 대체함.
...
## 2026-09-06 — 건물 루트 밖 장식 오브젝트가 오클루전 페이드에서 빠지던 버그 (진짜 마지막 원인)

### 사용자 리포트와 진단
CP34(FBX 임베디드 재질 수정) 이후에도 "플레이 해봤는데 똑같아"라는 피드백. 정확히 뭐가 그대로인지 물어봤더니 "Floor Line이 보이는거 같아"라는 구체적 단서를 줌.
`Floor Line 1~4` 오브젝트를 찾아보니 아파트 2개 위치(각각 X,Z는 건물과 동일, Y는 3/6/9/12로 층 높이에 맞춰 4개씩)에 정확히 겹쳐 배치되어 있었음 — `City`의 **직계 자식**(건물 루트의 자식이 아님)으로, 어두운 `Roof Dark` 재질의 평판. CP32 이전(아파트가 아직 저층 2행 모델이었을 때) 층 구분을 눈속임하려고 별도로 붙여둔 장식으로 추정됨.
**`PlayerOcclusionFader.BuildFadeState()`는 `root.GetComponentsInChildren<Renderer>()`로 건물 루트 하위만 훑기 때문에, 건물과 같은 자리에 있어도 형제(sibling) 오브젝트는 절대 페이드 대상에 포함되지 않음** — 이게 진짜 근본 원인이었음. CP34에서 고친 "일부 파츠가 잘못된 재질을 참조하던 버그"는 실존하는 문제였고 고칠 가치가 있었지만, 사용자가 실제로 보고 있던 "안 비침" 현상의 원인은 아니었던 것으로 보임(둘 다 존재했던 별개의 버그).

### 같은 문제의 전수 조사
"건물 루트 밖에 있으면서 건물과 겹쳐 배치된 오브젝트"라는 패턴으로 `City`의 모든 직계 자식을 각 건물의 실제 `BoxCollider` 크기 기준 반경으로 재검사 — 랜드마크 5종 전부에서 같은 문제 발견:
- `Police Station`: `Accent Stripe`(서명 색 스트라이프), `Light Bar`(지붕 경광등)
- `Fire Station`: `Garage Door`(차고 문)
- `Hospital`: `Cross Horizontal`/`Cross Vertical`(적십자 표시)
- `Pharmacy`: `Cross Horizontal`/`Cross Vertical`(녹십자 표시)

이 오브젝트들은 랜드마크 건물을 시각적으로 구분하기 위해 과거 세션에서 추가한 "서명 요소"(plan.md에 언급된 "재질+서명 요소로 차별화")인데, 처음부터 건물 루트가 아니라 `City` 최상위에 형제로 배치된 게 이번 버그의 씨앗이었음.

### 수정
- `Floor Line` 8개: 완전 삭제 — CP32의 새 5층 모델이 이미 벨트코스로 층 구분을 표시하므로 완전한 중복 잔재였음(단순 삭제가 정답, 재부모화 불필요).
- `Accent Stripe`/`Light Bar`/`Garage Door`/`Cross Horizontal`(x2)/`Cross Vertical`(x2) 총 7개: 각각 해당 건물 루트로 `Transform.SetParent(root, worldPositionStays: true)` — 월드 좌표/회전/스케일은 전혀 안 바뀌면서 계층 구조만 바뀌어, `PlayerOcclusionFader`가 자동으로 이들을 건물의 일부로 인식하게 됨(코드 수정 전혀 불필요 — 계층 구조만 고치면 기존 페이드 로직이 알아서 포함시킴).

### 교훈: "같은 자리에 있지만 다른 부모를 가진 오브젝트"는 항상 의심할 것
씬을 만들다 보면 "건물과 정확히 같은 위치에 있어야 하는 장식"을 편의상 건물 루트 밖(부모 없이 또는 다른 그룹 밑)에 배치하는 경우가 생기기 쉬움(이번 프로젝트에서 최소 8곳에서 반복됨). **하위 계층 기반으로 동작하는 시스템(오클루전 페이드, NavMesh 오프메시 링크, 그룹 활성화/비활성화 등)을 다룰 때는, "위치가 겹치는 오브젝트가 실제로 같은 부모 아래 있는지"를 항상 명시적으로 확인할 것** — 위치만 맞다고 계층까지 맞다고 가정하면 안 됨. 앞으로 새 건물/랜드마크 장식을 추가할 때는 처음부터 건물 루트의 자식으로 배치하는 습관을 들일 것.

### 검증
전수 재검사(각 건물의 실제 BoxCollider 크기 기준 반경)로 더 이상 페이드 안 되는 형제 오브젝트가 없음을 확인. 재부모화 전후 월드 좌표 완전 동일 확인. 씬 저장 후 디스크 강제 재로드로 영구 반영 확인(Floor Line 0개 남음). 콘솔 에러 0건.

## 2026-09-06 — 건물 모델 Blender 재제작 ("초등학생 그림같다" 피드백)

### 문제 진단
사용자가 기존 도시 건물이 "너무 초등학생이 그린 단순한 그림 같다"고 지적. Unity Play Mode에서 Police Station을 45도 각도로 실측 스크린샷 촬영해 확인한 결과, 원인은 두 가지였음.
1. 창문(`Window Frame 0~5`)/문이 전부 정면(+Z) 한 면에만 있고 좌/우/후면은 완전히 단색 평면 — 도시를 걸어다니며 대부분의 각도에서 보이는 면이 사실상 텅 빈 박스였음.
2. 창문 자체가 "프레임 박스 + 유리 박스" 두 장을 벽 표면에 겹쳐 놓은 것뿐이라, 실제로 파인 깊이(리세스)가 없어 조명 아래에서 그림자가 전혀 안 생기고 색깔만 다른 평면으로 보임.

### 새 모델 설계 (Blender 라이브 MCP 연결 상태에서 직접 작업)
- 이번엔 헤드리스 서브프로세스가 아니라 **연결된 Blender 인스턴스에서 직접 스크립트를 실행하고 뷰포트 렌더로 매 단계 확인**하면서 작업 — 과거 헤드리스 작업에서 반복됐던 "좌표를 감으로 계산하고 나중에 틀렸다는 걸 알게 되는" 패턴을 원천 차단.
- `Building Body`(솔리드 박스)에 4면(앞/뒤/좌/우) x 2행 x 3열 = 24개 창문 위치 컷터를 하나로 합쳐 Boolean Difference 한 번에 적용 — 실제로 파인 포켓(깊이 0.05)이 생김.
- `Window Frames`: 개구부보다 큰 사각 박스에서 개구부와 비슷한 크기의 구멍을 Boolean으로 뚫어 "속이 빈 테두리 링" 24개를 한 메시로 합침.
- `Window Glass`: 포켓 바닥(표면에서 0.025~0.05 안쪽)과 표면 사이 지점에 얇은 판을 배치해, 프레임 링의 구멍을 통해 리세스 안쪽에 있는 것처럼 보이게 함.
- `Door Frame`도 동일한 "속이 빈 링" 방식으로 재작업, `Door Panel`은 리세스 안쪽에, `Door Awning`(캐노피)을 문 위에 추가.
- `Belt Course`(중간 몰딩 띠, 4면 박스 4개를 합침), `Corner Pilasters`(모서리 트림 4개), `Roof Parapet`(지붕 가장자리를 감싸는 중공 테두리, Boolean으로 큰 박스에서 작은 박스를 뺌), `Roof Vent`(옥상 환기구, 실루엣용) 추가.
- 유닛 컨벤션은 기존 모델과 동일하게 유지(1 x 0.9 x 1, 문은 Blender -Y = Unity +Z 앞면) — 기존 20개 건물 인스턴스의 루트 트랜스폼(위치/회전/스케일)을 전혀 재계산할 필요 없이 모델만 교체하는 기존 패턴을 그대로 재사용하기 위함.

### [함정] Blender 노드 이름이 한국어로 로컬라이즈되어 있음
- `mat.node_tree.nodes.get("Principled BSDF")`가 계속 `None`을 반환해 `AttributeError`가 남 — 확인해보니 이 Blender 설치는 한국어 UI라 노드 이름 자체가 "프린시플드 BSDF"로 저장되어 있었음(노드의 `.name`은 UI 언어를 따라가고, `.type`은 언어와 무관하게 `'BSDF_PRINCIPLED'`로 고정). **앞으로 이 프로젝트의 Blender에서 노드를 찾을 때는 절대 이름으로 찾지 말고 항상 `node.type`으로 찾을 것.**

### [함정] "프레임"을 처음엔 속이 꽉 찬 판으로 만들어 개구부를 완전히 가림
- 창문 프레임과 문 프레임을 처음에 Boolean 없이 그냥 사각 박스(개구부보다 큰 사이즈)로 만들었더니, 그 박스가 통짜라서 뒤에 있는 리세스+유리/패널을 완전히 덮어버려 렌더링해보면 "그냥 색깔 있는 평평한 판"으로 보임 — 원래 고치려던 문제(프레임+유리 겹침으로 깊이감 없음)를 그대로 재현하는 바보 같은 실수. 뷰포트 렌더로 바로 발견해서(라이브 Blender의 장점) Boolean으로 테두리만 남기는 링 형태로 재작업. **앞으로 "프레임/테두리" 종류의 파츠를 만들 때는 반드시 렌더로 "안의 내용물이 실제로 보이는지"를 확인할 것 — 좌표만 맞다고 안심하지 말 것.**

### Unity 마이그레이션: 20개 건물 자동 탐색 + 파츠 교체
- City의 직계 자식 중 "Building Body"라는 이름의 자식을 가진 것을 전부 건물 루트로 자동 탐색(하드코딩 이름 목록 대신) — 정확히 20개(경찰서/소방서/병원/약국/아파트 등 랜드마크 포함) 발견.
- 각 건물의 기존 벽 색상 재질(`Building Body`의 `sharedMaterial`, 예: Police Blue/Fire Red/Hospital White/Building Beige/BlueGray/Brick/Concrete)을 먼저 기록 → 기존 18개 파츠(Building Body/Roof/Door Frame/Door Panel/Plinth/Stoop/Window Frame 0~5/Window Glass 0~5) 삭제 → 새 FBX 에셋의 13개 자식을 그대로 인스턴스화해 같은 루트의 자식으로 추가(로컬 위치/회전/스케일은 에셋에 저장된 값 그대로 복사) → `Building Body`에만 기록해둔 원래 벽 색상 재질을 재적용 + `BoxCollider`/`NavMeshModifier`/`Building` 레이어를 새로 추가.

### [치명적 함정] Play Mode인 줄 모르고 씬을 통째로 편집함
- 어두운 지도 버그 수정 검증 후 `manage_editor stop`을 호출했다고 생각했는데, 실제로는 그 뒤로도 계속 Play Mode였던 것으로 보임 — 한참 뒤(전체 마이그레이션 스크립트까지 다 실행한 뒤) `manage_editor play`를 호출했더니 "Already in play mode"라는 응답을 받고서야 깨달음. Unity는 원래 Play Mode 중 씬에 가한 변경사항을 Stop 시 전부 되돌리는 게 기본 동작이라, 이 세션에서 한 20개 건물 마이그레이션 작업 전체가 통째로 날아갈 뻔한 매우 위험한 상황이었음.
- 실제로 확인해보니: **구조적 변경(`DestroyImmediate`/`Instantiate`로 오브젝트를 새로 만들고 지운 것)은 Stop 후에도 살아남았지만, 그 이후 같은 Play 세션에서 고친 컴포넌트 필드 값(`BoxCollider.size`)은 Stop 후 확인해보니 실제로 예전 값으로 되돌아가 있었음.** 왜 이렇게 다르게 동작했는지 정확한 메커니즘은 알아내지 못함(이 MCP-for-Unity 연동 방식의 특성일 수도 있음) — **신뢰할 수 있는 규칙이 아니므로 절대 이 관찰에 의존하지 말 것.**
- **재발 방지 규칙(반드시 지킬 것)**: 씬을 변경하는 스크립트를 실행하기 전에는 항상 먼저 `UnityEngine.Application.isPlaying`을 확인하고, `true`면 반드시 `manage_editor stop`으로 먼저 끈 뒤에 진행할 것. 변경 직후에는 항상 `EditorSceneManager.MarkSceneDirty()` + `SaveScene()`으로 저장하고, 중요한 변경이면 `EditorSceneManager.OpenScene(path, OpenSceneMode.Single)`로 디스크에서 강제로 다시 열어 실제로 저장된 값인지 재확인할 것(이번에 실제로 이 방법으로 콜라이더 크기 수정이 처음엔 날아갔었다는 것을 잡아냄).

### [함정] 새 FBX 파츠의 localScale=100 때문에 콜라이더가 100배 커짐
- 새 `Building Body`를 기존 관례대로 `BoxCollider.size = (1, 0.9, 1)`로 그대로 만들었는데, 새로 임포트된 FBX의 자식 오브젝트가 (기존 모델과 달리) `localScale=(100,100,100)`을 갖고 있었음 — 렌더러 바운드로 확인해보면 시각적으로는 정확한 크기(1x0.9x1 상당)로 보이는데, 이는 Unity 임포터가 **메시 정점 데이터 자체를 미리 스케일 보정(약 0.01배)해서 굽기 때문**으로 추정됨(TEST 인스턴스로 에셋 루트 전체를 통째로 인스턴스화했을 때는 문제없이 정확한 크기가 나왔던 것과 대조적으로, 마이그레이션 스크립트에서는 에셋 루트를 거치지 않고 **개별 자식 오브젝트만 직접 인스턴스화**해서 루트에 있었을 보정용 스케일을 건너뛴 것이 원인으로 보임).
- 문제는 `BoxCollider`는 이런 메시 정점 보정의 영향을 받지 않고 트랜스폼의 `lossyScale`을 그대로 곱해서 월드 크기를 계산한다는 것 — 그 결과 `size=1` x `localScale=100` x `부모(건물 루트) 스케일=6` = 월드 기준 600유닛짜리 콜라이더가 생겨버림(원래 의도한 6유닛의 100배). 실제로 `collider.bounds`를 찍어보고 나서야(Extents 300 등 비정상적인 숫자) 발견함.
- 해결: 하드코딩된 크기 대신 `body.GetComponent<MeshFilter>().sharedMesh.bounds`(메시 자신의 로컬 바운드, 스케일과 무관하게 항상 정확함)를 그대로 `BoxCollider.center`/`size`에 대입 — 스케일 값이 얼마든 상관없이 항상 실제 시각적 메시와 정확히 일치하게 만드는 가장 안전한 방법. **앞으로 새로 임포트하거나 교체한 모델에 콜라이더를 붙일 때는 절대 예전 모델이나 다른 에셋의 크기 값을 재사용하지 말고, 반드시 그 오브젝트 자신의 `sharedMesh.bounds`를 읽어서 쓸 것** — FBX 임포트 시 유닛 스케일 보정 방식은 에셋/익스포트 설정마다 달라질 수 있어서 "지난번엔 1이었으니까 이번에도 1이겠지"라는 가정이 위험함.
- 다행히 이 버그가 있던 (100배 커진) 콜라이더로 NavMesh를 재굽지는 않았음(콜라이더를 만들자마자 바로 다음 단계에서 크기를 확인하다가 발견) — 만약 그 상태로 NavMesh를 구웠다면 도로 전체가 막히는 훨씬 큰 사고로 이어졌을 것.

### 검증
재질 슬롯 null 0건(20개 건물, 총 260개 렌더러 전수 검사), 파츠 개수 전부 정확히 13개, `Building Body`의 레이어("Building")/`BoxCollider` 정상 20/20. 콜라이더 크기 수정 후 씬 저장 → 디스크에서 강제 재로드해 실제 저장값 재확인(통과). Play Mode에서 `PlayerOcclusionFader.Update()`를 리플렉션으로 강제 호출 — 플레이어를 건물 뒤로 옮기면 알파 0.25로 정상 페이드, 건물 시야가 없는 열린 도로로 옮기면 알파 1로 정상 복귀. 콜라이더 월드 크기가 기존과 동일(6x4.05x6 등, 건물마다 다름)해서 기존에 구워둔 NavMesh를 그대로 재사용 가능(재굽기 불필요). 컴파일 에러 0건, 콘솔 에러 0건.

## 2026-09-06 — 세션 종료 (버그 수정 위주의 하루 요약, 다른 컴퓨터에서 이어서 작업 예정)

어제(2026-09-05) 만든 도시 구역을 사용자가 실제로 플레이해보면서 나온 피드백들을 하나씩 잡은 하루. CP28~CP35 총 8개 체크포인트, 전부 커밋·검증·푸시 완료.

### 오늘 완료된 것 (커밋 순서대로, 전부 `origin/main`에 push 완료)
1. Unity 6.3.23f1로 에디터/패키지 버전 동기화(다른 컴퓨터에서 열며 자동 갱신된 것).
2. 전체지도(M키) 어둡게 찍히는 버그 — `DayNightCycle` 태양 회전 공식이 밝기 지표와 90도 위상 어긋남 수정.
3. 도시 건물을 Blender로 재제작 — 4면 전체에 실제 창문 리세스 + 모서리 필라스터 + 벨트코스 + 파라펫(`BuildingDetailed.fbx`).
4. 도로/바닥이 완전히 검게 렌더링되던 버그 — 타일링 텍스처가 조명과 무관하게 순수 검정으로 샘플링되던 것을 발견, 텍스처 제거 후 플랫 컬러로 통일.
5. 아파트 건물 전용 5층 모델 제작(`BuildingDetailedTall.fbx`) — 공용 저층 모델을 세로로 긴 아파트에 그대로 쓰다 깨져 보이던 문제.
6. 건물 오클루전 페이드 알파 0.25→0.15→0.06 단계적 하향.
7. 건물 파츠 일부가 FBX 임베디드 재질(Blender 프리뷰용과 이름이 겹쳤던 것)을 참조하던 버그 수정.
8. **Floor Line/랜드마크 서명 요소가 건물 루트 밖(City 형제)에 있어서 오클루전 페이드 그룹에서 빠지던 구조적 버그** — 이게 "건물이 반투명해져도 뭔가 그대로 보인다"는 사용자 피드백의 진짜 근본 원인이었음(6, 7번은 그 자체로 실존하는 버그라 고칠 가치가 있었지만, 사용자가 실제로 보고 있던 현상의 원인은 8번이었음). 사용자가 최종 확인 완료("좋아 잘되었어").

### 오늘의 핵심 교훈 (다음 세션에서 재발 방지)
- **씬을 스크립트로 편집하기 전엔 항상 `Application.isPlaying`부터 확인할 것** — Play Mode 중 편집은 Stop 시 예측 불가능하게 되돌아갈 수 있음(이번에 몇 번 겪음). 편집 후엔 `SaveScene()` + 디스크 재로드로 실제 저장 확인하는 습관이 여러 번 실수를 잡아줬음.
- **새 모델에 콜라이더를 붙일 때는 절대 다른 모델의 크기 값을 재사용하지 말고 `sharedMesh.bounds`를 읽을 것** — FBX 임포트 유닛 스케일은 에셋마다 다를 수 있음(아파트 모델 제작 때는 이 교훈을 실제로 적용해 재발 안 시킴 — 좋은 사례).
- **Blender에서 프리뷰용 재질을 만들 때 프로젝트의 실제 재질과 이름이 절대 겹치지 않게 할 것** — 이름이 겹치면 FBX 임포트 시 임베디드 서브에셋이 헷갈리게 섞여 들어감.
- **"위치가 겹치는 오브젝트가 실제로 같은 부모 아래 있는지"는 항상 명시적으로 확인할 것** — 하위 계층 기반으로 동작하는 시스템(오클루전 페이드 등)에서, 위치만 맞고 계층이 다른 오브젝트는 조용히 빠짐. 이번 세션 최대 교훈.
- **렌더링 버그를 헤드리스로 진단할 때, 실제 게임 카메라 앵글(탑다운)과 다른 각도로 테스트하면 결과가 오도할 수 있음** — 가능하면 실제 게임의 카메라 오프셋과 비슷하게 맞출 것. 또한 여러 조건을 비교할 땐 반드시 하나의 스크립트 실행 안에서 카메라를 한 번만 배치하고 연속으로 캡처할 것(실시간 시스템이 호출 사이에 상태를 되돌려놓을 수 있음).

### 최신 상태
- 최신 커밋: `1aabd26`, 브랜치 `main`, `origin/main`과 완전히 동기화됨. `git status --short`에 Packages/ProjectSettings의 사소한 로컬 환경 차이(line-ending만 바뀌는 자동 터치 파일들)만 남아있고 이는 매 세션 공통으로 커밋 대상 아님.
- 콘솔 에러 0건 상태 유지. 사용자가 오늘 수정한 모든 항목(전체지도 밝기, 도로 색상, 건물 디테일, 아파트 층수, 오클루전 페이드)을 실제 플레이로 확인 완료.

### 다음 세션 시작 시 순서
1. `plan.md` 최상단 "현재 상태 요약" 섹션 먼저 읽기.
2. `git log`/`git status`로 원격과 로컬 상태 일치 확인.
3. Unity 에디터 컴파일 에러 확인 + Play Mode 베이스라인 재확인(이동/사격/근접무기/인벤토리/미니맵/상태창/건물 입구 전환/낮밤전환/좀비 스폰/오클루전 페이드).
4. 사용자에게 다음 방향 확인 — `checklist.md` 맨 아래 "다음 단계" 섹션에 후보들 정리되어 있음(도시 격자 추가 확장 vs 광산 구역 시작 vs 대량 좀비 스폰 성능 실측 등).
