...
- [x] **위치 조정(사용자 요청)**: 각 라벨의 피벗을 (0.5,0.5)로, anchoredPosition을 (0,0)으로 맞춰 글자 중심점이 원 둘레(반지름 80 지점)에 정확히 걸치도록 배치(기존엔 중심에서 8px 안쪽으로 치우쳐 있었음).
      Verify: 스크린샷 크롭 확대로 N/S/E/W 글자가 원 테두리에 걸쳐 있음을 확인.

## CP5. 통합 검증
- [x] Unity 컴파일 에러 0건.
- [x] 좀비 스폰 후에도 미니맵에 좀비가 전혀 안 보임 (레이어 격리로 보장).
- [x] 총기 발사가 좀비(Enemy 레이어)에게 정상 명중 (레이캐스트 회귀 테스트 통과).
- [x] 실제 에디터에서 사람이 직접 플레이하며 "미니맵이 이동을 따라오는지" 최종 확인 — 사용자가 직접 플레이 테스트 완료.

## Slice 1 완료
위 CP1~CP6 전부 검증 완료. 다음은 `plan.md`의 Slice 2(레이더)로 진행.
